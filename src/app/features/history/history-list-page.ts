...
@Component({
  ...
  template: `
    <main class="page">
      ...
      <form class="card filters" (submit)="onSubmit($event)" aria-label="Filtrar historial de entrenamientos">
      ...
      @if (loading()) {
        ...
        <section class="grid" aria-label="Historial de entrenamientos">
          @for (workout of workouts(); track workout.id) {
            <article class="card workout-card">
              <div><p class="eyebrow">{{ formatDate(workout.performedOn) }}</p><h2>{{ workout.routineName }}</h2><p>{{ workout.notes || 'Sin notas del entrenamiento.' }}</p><dl><div><dt>Total</dt><dd>{{ workout.plannedCount }}</dd></div><div><dt>Completados</dt><dd>{{ workout.completedCount }}</dd></div><div><dt>Omitidos</dt><dd>{{ workout.skippedCount }}</dd></div><div><dt>Completitud</dt><dd>{{ workout.completionPercentage }}%</dd></div></dl></div>
